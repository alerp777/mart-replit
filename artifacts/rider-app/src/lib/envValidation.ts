 ...
 * rider app. Warnings are emitted only in development mode (import.meta.env.DEV)
...
function _buildRiderEnv(): RiderEnv {
...
export const riderEnv: RiderEnv = _buildRiderEnv();

/**
 * True in Vite dev-server builds; false in production bundles.
 * Exported so callers don't read import.meta.env.DEV directly.
 */
export const riderIsDev: boolean = import.meta.env.DEV;

/** Convenience alias used in main.tsx for the startup audit side-effect. */
export function auditRiderEnv(): RiderEnv {
  return riderEnv;
}

/**
 * Returns the computed API base string consumed by api.ts and socket.tsx.
 * Capacitor native builds use the absolute VITE_API_BASE_URL; web-proxy
 * mode uses the relative `/api` path served through the Vite dev proxy.
 */
export function getRiderApiBase(): string {
  return riderEnv.isCapacitor && riderEnv.apiBaseUrl
    ? `${riderEnv.apiBaseUrl.replace(/\/+$/, "")}/api`
    : `/api`;
}

/**
 * Returns the socket.io origin for Capacitor builds, or undefined for
 * web-proxy mode (where the connection is relative to the page origin).
 */
export function getRiderSocketOrigin(): string | undefined {
  return riderEnv.isCapacitor && riderEnv.apiBaseUrl
    ? riderEnv.apiBaseUrl.replace(/\/+$/, "")
    : undefined;
}
